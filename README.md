...
AiMall 是一个包含管理后台、商城前台和 Django 后端的电商推荐实验项目，支持真实业务链路下的个性化推荐与实验数据采集。
...
项目内置推荐实验样本生成与三算法对比命令：

```bash
backend/.venv/bin/python backend/manage.py recommendation_experiment_lab \
  --customers 36 \
  --actions 40 \
  --seed 20260408 \
  --clear-all-behaviors
...
```bash
backend/.venv/bin/python backend/manage.py recommendation_experiment_lab --skip-seed
```
...
- 建议固定随机种子并保留每次实验的参数、指标截图与实验记录编号
